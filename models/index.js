'use strict';

const fs = require('fs');
const path = require('path');
const Sequelize = require('sequelize');
const basename = path.basename(__filename);
const env = process.env.NODE_ENV || 'development';
const config = require(__dirname + '/../config/config.js')[env];
const db = {};

/**
 * Initialize Sequelize instance
 * If `use_env_variable` is defined in the config, use the environment variable to initialize Sequelize.
 * Otherwise, use the configuration settings.
 */
let sequelize;
if (config.use_env_variable) {
 sequelize = new Sequelize(process.env[config.use_env_variable], config);
} else {
 sequelize = new Sequelize(config.database, config.username, config.password, config);
}

/**
 * Read all model files in the current directory, excluding this file.
 * Import each model and add it to the `db` object.
 */
fs
 .readdirSync(__dirname)
 .filter(file => {
  return (file.indexOf('.') !== 0) && (file !== basename) && (file.slice(-3) === '.js');
 })
 .forEach(file => {
  const model = require(path.join(__dirname, file))(sequelize, Sequelize.DataTypes);
  db[model.name] = model;
 });

/**
* Call the associate method on each model, if it exists.
* This sets up any associations (relationships) between models.
*/
Object.keys(db).forEach(modelName => {
 if (db[modelName].associate) {
  db[modelName].associate(db);
 }
});

/**
 * Export the `db` object containing all models and the Sequelize instance.
 */
db.sequelize = sequelize;
db.Sequelize = Sequelize;

module.exports = db;